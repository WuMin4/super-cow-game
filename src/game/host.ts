...
export class GameHost {
  ...
  addPlayer(id: string, _name: string) {
    if (Object.keys(this.state.players).length >= 6) return;
  ...
  handleMessage(peerId: string, data: any) {
    ...
    } else if (data.type === 'KICK_PLAYER' && peerId === 'host') {
      this.removePlayer(data.targetId);
  ...
  updatePhysics() {
    for (const id in this.state.players) {
      ...
      if (p.x < 0.4) { p.x = 0.4; p.vx = 0; }
      if (p.x > 44.6) { p.x = 44.6; p.vx = 0; }
  ...
  generateSelectableBlocks() {
    ...
    const extraBombs = Math.min(2, this.state.consecutiveFails || 0);
  ...
  checkPlacementComplete() {
    ...
    if (allPlaced && this.state.phase === 'PLACEMENT') {
      ...
      const timer = setInterval(() => {
        count--;
        if (count > 0) {
          this.state.countdownValue = count;
        } else {
          clearInterval(timer);
  ...
  checkRoundEnd() {
    ...
    if (allDone && this.state.phase === 'PLAY') {
      ...
      const finished = players.filter(p => p.hasFinished);
  ...
  calculateScores() {
    ...
    const trapPoints: Record<string, number> = {};
    players.forEach(p => trapPoints[p.id] = 0);
    
    players.forEach(p => {
      if (p.isDead && p.deathCauseOwner && p.deathCauseOwner !== p.id) {
        trapPoints[p.deathCauseOwner] += 0.2 * multiplier;
      }
    });
    
    for (const id in trapPoints) {
      if (trapPoints[id] > 0) {
        const finalTrapPts = Math.min(0.5, trapPoints[id]);
        results[id].points += finalTrapPts;
        results[id].details.push(`陷阱击杀: +${finalTrapPts.toFixed(2)} (上限0.5)`);
      }
    }
