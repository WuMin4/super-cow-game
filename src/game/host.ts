...
export class GameHost {
  ...
  getInitialBlocks(): Block[] {
    return [
      { id: 'start', x: 0, y: 16, w: 4, h: 4, type: 'START', ownerId: null },
      { id: 'end', x: 41, y: 16, w: 4, h: 4, type: 'END', ownerId: null },
      { id: 'flag', x: 42, y: 14, w: 1, h: 2, type: 'FLAG', ownerId: null }
    ];
  }
  ...
  updatePhysics() {
    for (const id in this.state.players) {
      ...
      let friction = 0.9;
      if (p.onGround) {
        friction = p.groundType === 'ICE' ? 0.98 : 0.8;
      }

      const accel = p.onGround ? 0.05 : 0.02;
      ...
      if (p.y > MAP_HEIGHT + 2 || p.x < -2 || p.x > MAP_WIDTH + 2) {
        this.die(p, null);
      }
    ...
    if (block.type === 'BOMB') {
      cells.forEach(c => {
        ...
        this.state.blocks = this.state.blocks.filter(b => {
          if (['START', 'END', 'FLAG'].includes(b.type)) return true;
          const eRect = { l: b.x, r: b.x + b.w, t: b.y, b: b.y + b.h };
