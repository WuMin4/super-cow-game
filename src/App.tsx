...
export default function App() {
  ...
  useEffect(() => {
    ...
    ctx.fillRect(0, 18 * TILE_SIZE, 4 * TILE_SIZE, 7 * TILE_SIZE);
    ctx.fillRect(41 * TILE_SIZE, 18 * TILE_SIZE, 4 * TILE_SIZE, 7 * TILE_SIZE);
      ...
      (Object.values(gameState.players) as Player[]).forEach(p => {
        if (!p.hasPlaced && p.selectedBlockIndex !== null) {
          ...
          if (b) {
            ...
            // Draw player name above the hovering block
            const minX = Math.min(...cells.map(c => c.x));
            const minY = Math.min(...cells.map(c => c.y));
            const maxX = Math.max(...cells.map(c => c.x + c.w));
            const centerX = (minX + maxX) / 2 * TILE_SIZE;
            const topY = minY * TILE_SIZE - 5;
            
            ctx.fillStyle = p.color;
            ctx.font = '12px Inter, sans-serif';
            ctx.textAlign = 'center';
            ctx.fillText(p.name, centerX, topY);
      ...
      <div className="w-full max-w-[1800px] mb-4 bg-gray-800 p-4 rounded-lg shadow-lg">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="text-white">阶段: {
            ...
            gameState?.phase === 'COUNTDOWN' ? '准备开始' :
      ...
      <div className="relative">
        ...
        {gameState?.phase === 'LOBBY' && (
          <div className="absolute inset-0 bg-black/50 flex flex-col items-center justify-center rounded-lg p-6">
            <h2 className="text-white text-3xl font-bold mb-4">等待玩家加入... ({Object.keys(gameState.players).length}/6)</h2>
            ...
            <div className="flex flex-wrap gap-4 mb-8 justify-center">
              {(Object.values(gameState.players) as Player[]).map(p => (
                <div key={p.id} className="flex items-center bg-gray-800 rounded-full pl-4 pr-2 py-1 border border-gray-600">
                  <span className="text-white font-bold mr-3" style={{ color: p.color }}>{p.name}</span>
                  {isHost && p.id !== myId && (
                    <button 
                      onClick={() => hostRef.current?.handleMessage('host', { type: 'KICK_PLAYER', targetId: p.id })}
            ...
            {isHost && (
              <div className="flex gap-4">
                <button onClick={() => hostRef.current?.handleMessage('host', { type: 'ADD_AI' })} className="bg-green-600 text-white px-8 py-3 rounded-lg text-xl font-bold hover:bg-green-700 transition disabled:opacity-50" disabled={Object.keys(gameState.players).length >= 6}>
                  添加AI
                </button>
        ...
        {gameState?.phase === 'COUNTDOWN' && (
          <div className="absolute inset-0 bg-black/30 flex items-center justify-center rounded-lg z-50">
            <div className="text-white text-9xl font-bold animate-bounce drop-shadow-[0_0_15px_rgba(255,255,255,0.8)]">
