  ...
  updateAI() {
    ...
    if (this.state.phase === 'SELECTION') {
      ...
      aiPlayers.forEach(ai => {
        if (!ai.hasPlaced && ai.selectedBlockIndex !== null) {
          if (Math.random() < 0.05) {
            ...
            // Find highest block at this X to place above it
            let highestY = 23; // Default near ground
            ...
            // If it's a trap, maybe place it right on the ground/block
            // If it's a platform, maybe place it higher to make a bridge
            if (block.type === 'BRICK' || block.type === 'ICE') {
              targetY -= Math.floor(Math.random() * 3) + 1; // 1 to 3 blocks higher
            } else if (block.type === 'SPRING' || block.type === 'FAN') {
              // place on top of highest
            }
            ...
            this.handleMessage(ai.id, { type: 'PLACE_BLOCK' });
          }
        }
      });
      ...
      aiPlayers.forEach(ai => {
        ...
        if (aiCenterX < targetX - 0.5) {
          ai.inputs.right = true;
        } else if (aiCenterX > targetX + 0.5) {
          ai.inputs.left = true;
        }

        // Jump logic: Look ahead
        const lookAheadX = ai.inputs.right ? ai.x + 1.5 : (ai.inputs.left ? ai.x - 1.5 : ai.x);
        
        // Check if there's a block blocking the way or a gap
        let gapAhead = true;
        let obstacleAhead = false;
        
        for (const b of this.state.blocks) {
          // Check ground ahead
          if (lookAheadX >= b.x && lookAheadX <= b.x + b.w) {
            if (b.y >= ai.y + 0.5 && b.y <= ai.y + 3) {
              gapAhead = false;
            }
          }
          // Check obstacle ahead
          if (b.y < ai.y + 0.9 && b.y + b.h > ai.y - 0.9) {
            if (ai.inputs.right && b.x > ai.x && b.x < ai.x + 1.5) obstacleAhead = true;
            if (ai.inputs.left && b.x + b.w < ai.x && b.x + b.w > ai.x - 1.5) obstacleAhead = true;
          }
        }

        if ((gapAhead || obstacleAhead) && ai.onGround) {
          // Add some randomness to jump timing
          if (Math.random() < 0.8) {
            ai.inputs.jump = true;
          }
        }

        // Hold jump for a bit to jump higher
        if (!ai.onGround && ai.vy < 0 && Math.random() < 0.9) {
          ai.inputs.jump = true;
        }
      });
    }
